...
CDF.plot = ggplot() +
  geom_step(data = bind_rows(tibble(x = -2, CDF = 0), ggdat, tibble(x = 5, CDF = 1)),
            aes(x = x, y = CDF)) +
  ...
  geom_hline(yintercept = 0) +
  scale_x_continuous("x", breaks = 0:3) +
  coord_cartesian(xlim = c(-1, size + 1)) +
...
PMF.plot = ggplot() +
  ...
  scale_color_manual(values = c("FALSE" = "black", "TRUE" = "darkorchid"),
                     guide = "none") +
  scale_x_continuous("x",breaks=0:x.limit) + 
  ylab("Probability") + 
  theme_bw()

CDF.plot = ggplot() +
  geom_step(data = bind_rows(tibble(x = -2, CDF = 0), ggdat, tibble(x = x.limit + 2, CDF = 1)),
            aes(x=x, y=CDF)) +
  geom_point(data=ggdat, aes(x = x, y = CDF)) +
  geom_point(data=filter(ggdat, x == 1),aes(x = x, y = CDF),
             color = "darkorchid", size = size) +
  geom_hline(yintercept = 0) +
  scale_x_continuous("x", breaks = 0:x.limit) +
  coord_cartesian(xlim = c(-1, x.limit + 1)) +
  ylab("Cumulative Probability") +
  theme_bw()
...
**Solution**
```{r}
win.prob = dbbinom(2,size,alpha=alpha,beta=beta) + 
  dbbinom(3,size,alpha=alpha,beta=beta)
(win.prob)
```
...
ggdat = tibble(x= 0:size) |>
  mutate(
        PMF = dbbinom(x,size=size,alpha=alpha,beta=beta),
        CDF = pbbinom(x,size=size,alpha=alpha,beta=beta)
        )
...
CDF.plot = ggplot() +
  geom_step(data = bind_rows(tibble(x = -1, CDF = 0), ggdat, tibble(x = size + 1, CDF = 1)),
            aes(x = x, y = CDF)) +
  geom_point(data = ggdat, aes(x = x, y = CDF)) +
  geom_point(data = filter(ggdat, x == 1), aes(x = x, y = CDF),
             color = "darkorchid", size = 3) +
  geom_hline(yintercept = 0) +
  scale_x_continuous("x", breaks = 0:size, limits = c(0, size)) +
  ylab("Cumulative Probability") +
  theme_bw()
...
(binom.exp = sum(x * dbinom(x, size, p)))
(bbinom.exp = sum(x * dbbinom(x, size, alpha, beta)))
```
...
**Variance**
```{r}
(binom.var.numerical = sum((x - binom.exp)^2 * dbinom(x, size, p)))
(binom.var.formula   = size * p * (1-p))

(bbinom.var.numerical = sum((x - bbinom.exp)^2 * dbbinom(x, size, alpha, beta)))
(bbinom.var.formula   = (size * alpha * beta * (alpha + beta + size)) /
...
```{r}
#| label: fig-altbinom-pmf-thetas
#| fig-cap: "PMF of Altham's Multiplicative Binomial distribution with n=3, p=0.70 for theta=0.5, 1, and 1.5."
#| echo: false

(plot.pmf(size,p,thetas[1]) + ggtitle("theta = 0.5")) |
(plot.pmf(size,p,thetas[2]) + ggtitle("theta= 1"))   |
(plot.pmf(size,p,thetas[3]) + ggtitle("theta= 1.5"))
```
...
```{r}
#| label: fig-altbbinom-pmf-thetas
#| fig-cap: "PMF of Altham's Multiplicative Beta-Binomial distribution with n=3, alpha=7, beta=3 for theta=0.5, 1, and 1.5."
#| echo: false

(plot.altbbinom(size,alpha,beta,thetas[1]) + ggtitle("theta = 0.5")) |
(plot.altbbinom(size,alpha,beta,thetas[2]) + ggtitle("theta = 1"))   |
(plot.altbbinom(size,alpha,beta,thetas[3]) + ggtitle("theta = 1.5"))
```
