...
format-links: [pdf]
include-in-header: #not at all my idea, got from Gemini for text wraparound
  text: |
    \usepackage{fvextra}
...
First, locate the source data within the nature article linked at `Source Data`. From the provided excel document, save the excel sheet `Fig. 3B` to a separate excel document. Within this document, first delete the empty top row, and the Primary and THP1 cells. Next, cut and paste the entire data contents of the two right columns below the last entry on the left columns. After this, the data should be loaded into a csv file with two columns, I renamed the column headers to be `sample` and `count` respectively.
...
```{r}
dat.kitsberg = read_csv("fig3b.csv")
dat.kitsberg.cleaned = dat.kitsberg |> 
  separate_wider_delim("sample","_",names=c("SampleType","CellType","Treatment"))
...
```{r}
dat.kitsberg.cleaned = dat.kitsberg.cleaned |>
  mutate(CellType = case_when((CellType == 'MDM' | CellType == 'mac') ~ 'mac', CellType == 'mono' ~ 'mono', CellType == 'PDGFR' ~ 'PDGFR'))

unique(dat.kitsberg.cleaned$CellType)
```
...
```{r}
dat.kitsberg.cleaned = dat.kitsberg.cleaned |>
  mutate(SampleType = case_when(SampleType == "primary" ~ "Healthy Donor", SampleType == "THP1" ~ "THP-1")) |>
  mutate(CellType = case_when(CellType == "mono" ~ "Monocytes", CellType == "mac" ~ "Macrophages")) |>
  mutate(Treatment = "12 Hours Post-Infection") #all uninfected already removed
...
dat.kitsberg.cleaned = dat.kitsberg |>
  separate_wider_delim("sample","_",names=c("SampleType","CellType","Treatment")) |>
  filter(Treatment != "uninfected") |> #remove uninfected
  mutate(CellType = case_when((CellType == 'MDM' | CellType == 'mac') ~ 'mac', CellType == 'mono' ~ 'mono')) |> #rename MDM to mac
  filter(CellType %in% c("mono","mac")) |> #only keep mono and mac
  #prettier data entry -------------
  mutate(SampleType = case_when(SampleType == "primary" ~ "Healthy Donor", SampleType == "THP1" ~ "THP-1")) |>
  mutate(CellType = case_when(CellType == "mono" ~ "Monocytes", CellType == "mac" ~ "Macrophages")) |>
  mutate(Treatment = "12 Hours Post-Infection") #all uninfected already removed
...
#| echo: false
#| label: fig-3b
#| fig-cap: HCMV Genomes in Nucleus vs Sample Type
width = 1
dat.kitsberg.cleaned$countjitter <- dat.kitsberg.cleaned$count + 
  runif(nrow(dat.kitsberg.cleaned), -0.25, 0.25)
plot = ggplot(dat.kitsberg.cleaned,aes(x=CellType,y=count)) +
  geom_violin(position=position_dodge(width)) + 
  labs(x="Sample Type",y="HCMV Genomes in Nucleus") +
  scale_color_manual("Cell Type", values=c("black", "gray")) +
...
**Solution**
```{r}
dat.bal = read_csv("ShipValuation-Bal25.csv")[1:1819, ]
dat.bal.cleaned = dat.bal |> 
  mutate(price = (10^SPLog) / 1000000)

head(dat.bal.cleaned$price)
```
...
dat.bal.cleaned = dat.bal.cleaned |>
  mutate(OriginCountry = case_when(Japan == 1 ~ "Japan", `China, People's Republic Of` == 1 ~ "China, People's Republic Of", Philippines == 1 ~ "Philippines", `Korea, South` == 1 ~ "Korea, South", TRUE ~ "Others Country Build"))
unique(dat.bal.cleaned$OriginCountry)
```
...
```{r}
dat.bal.cleaned = dat.bal.cleaned |>
  mutate(Manufacturer = case_when(Caterpillar == 1 ~ "Caterpillar", `MAN-B&W` == 1 ~ "Man-B&W", Wartsila == 1 ~ 'Wartsila', Mitsubishi == 1 ~ 'Mitsubishi'))
unique(dat.bal.cleaned$Manufacturer)
```
...
#| code-overflow: wrap
dat.bal.cleaned = dat.bal.cleaned |>
  mutate(PI.club = case_when(`Japan P&I Club` == 1 ~ "Japan P&I Club", Gard == 1 ~ "Gard", `North of England` == 1 ~ 'North of England',`West of England` == 1 ~ "West of England", `UK P&I` == 1 ~ "UK P&I", `Britannia P&I` == 1 ~ "Britannia P&I", `London P&I Club` == 1 ~ "London P&I Club", Skuld == 1 ~ "Skuld", `Steamship Mutual` == 1 ~ "Steamship Mutual", Others == 1 ~ "Others", Unknown == 1 ~ "Unknown"))
unique(dat.bal.cleaned$PI.club)
```
...
```{r}
dat.bal.cleaned = dat.bal.cleaned |>
  mutate(Class = case_when(`Nippon Kaiji Kyokai (IACS)` == 1 ~ "Nippon Kaiji Kyokai (IACS)", `Lloyd's Register` == 1 ~ "Lloyd's Register", `Bureau Veritas (IACS)` == 1 ~ "Bureau Vertias (IACS)",`Det Norske Veritas (IACS)` == 1 ~ "Det Norske Veritas (IACS)", `American Bureau of Shipping (IACS)` == 1 ~ "American Bureau of Shipping (IACS)", `Others Class` == 1 ~ "Others Class", `With Two Classes` == 1 ~ "With Two Classes", Disclassed == 1 ~ "Disclassed"))

unique(dat.bal.cleaned$Class)
```
...
**Note**: There is a minor issue in the above text, the corect flag state is `Hong Kong, China`

**Solution**

```{r}
dat.bal.cleaned = dat.bal.cleaned |>
  mutate(FlagState = case_when(Panama == 1 ~ "Panama", `Marshall Islands` == 1 ~ "Marshall Islands", Liberia == 1 ~ "Liberia", Malta == 1 ~ "Malta", `Hong Kong, China` == 1 ~ "Hong Kong, China", Singapore == 1 ~ "Singapore", `Others Flag` == 1 ~ "Others Flag", `With two flags` == 1 ~ "With two flags"))

unique(dat.bal.cleaned$FlagState)
```
...
#| echo: false
#| label: fig-PI-Price
#| fig-cap: "Ship Price vs P&I Club Membership"
ggplot(data=dat.bal.cleaned) +
  geom_boxplot(aes(x=PI.club,y=price)) +
  labs(x="P&I Club", y= "Ship Price (Millions USD)") +
...
#| echo: false
#| label: tbl-PI-Price
#| tbl-cap: "Ship Price (Millions USD) by P&I Club"
dat.bal.cleaned |>
  rename("P&I Club" = PI.club) |>
  group_by(`P&I Club`) |>
  summarize(
...
As seen in both @fig-PI-Price and @tbl-PI-Price, P&I clubs insure ships of a similar distribution of prices, suggesting that insurers tend to not only diversify their portfolios, but also have essentially reached a consensus on the appropriate basket of ships.
...
```{r}
#| echo: false
#| label: fig-Price-LIBOR
#| fig-cap: "Ship Price vs LIBOR"
#| scale: "70%"
ggplot(data=dat.bal.cleaned,aes(x=LIBOR,y=price)) +
  geom_point() +
  geom_smooth(method="lm",se=FALSE,color="red", formula = y ~ x) +
  labs(x="London InterBank Offer Rate", y="Ship Price (Millions USD)")
```

```{r}
#| echo: false
#| label: tbl-Price-LIBOR
#| tbl-cap: Correlation of Ship Price and LIBOR
dat.bal.cleaned |>
  summarize(
    pearson = cor(x=LIBOR,y=price,method="pearson"),
    spearman = cor(x=LIBOR,y=price,method="spearman"),
    kendall = cor(x=LIBOR,y=price,method="kendall")
  ) |>
  knitr::kable(digits=2)
```

@fig-Price-LIBOR and @tbl-Price-LIBOR show a weak positive linear correlation between price and average interest rates over the last 3 months. As with any good, we expect its price to increase with the interest rate, which represents the price to borrow capital. It's clear from @fig-Price-LIBOR that ship prices also vary independently of the `LIBOR`, which is plausible given variations in ship sizes, age, and capacity.
